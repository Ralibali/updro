...
-- project_outcomes: buyer-reported outcome per project
CREATE TABLE IF NOT EXISTS public.project_outcomes (
  ...
  selected_supplier_id uuid REFERENCES public.supplier_profiles(id) ON DELETE SET NULL,
  actual_value_sek numeric(12,2),
  comment text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT project_outcomes_outcome_chk CHECK (outcome IN ('hired','still_deciding','not_proceeding')),
  CONSTRAINT project_outcomes_value_chk CHECK (actual_value_sek IS NULL OR (actual_value_sek >= 0 AND actual_value_sek <= 100000000)),
  CONSTRAINT project_outcomes_comment_chk CHECK (comment IS NULL OR char_length(comment) <= 1500),
  CONSTRAINT project_outcomes_hired_offer_chk CHECK (
    (outcome = 'hired' AND selected_offer_id IS NOT NULL)
    OR (outcome <> 'hired' AND selected_offer_id IS NULL AND selected_supplier_id IS NULL)
  )
);

CREATE INDEX IF NOT EXISTS idx_project_outcomes_outcome ON public.project_outcomes(outcome);
CREATE INDEX IF NOT EXISTS idx_project_outcomes_created_at ON public.project_outcomes(created_at DESC);

GRANT SELECT ON public.project_outcomes TO authenticated;
GRANT ALL ON public.project_outcomes TO service_role;

ALTER TABLE public.project_outcomes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Buyers can read their own outcome" ON public.project_outcomes;
CREATE POLICY "Buyers can read their own outcome"
  ON public.project_outcomes FOR SELECT
  TO authenticated
  USING (buyer_id = auth.uid());

DROP POLICY IF EXISTS "Admins can read all outcomes" ON public.project_outcomes;
CREATE POLICY "Admins can read all outcomes"
  ON public.project_outcomes FOR SELECT
  TO authenticated
  USING (public.is_admin(auth.uid()));

DROP TRIGGER IF EXISTS trg_project_outcomes_updated_at ON public.project_outcomes;
CREATE TRIGGER trg_project_outcomes_updated_at
  BEFORE UPDATE ON public.project_outcomes
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- RPC: report_project_outcome
...
REVOKE ALL ON FUNCTION public.report_project_outcome(uuid, text, uuid, numeric, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.report_project_outcome(uuid, text, uuid, numeric, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.report_project_outcome(uuid, text, uuid, numeric, text) TO authenticated;
