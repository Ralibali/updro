-- Guest publish no longer requires a name. Email remains the contact gate.
...
BEGIN
  ...
  VALUES (lower(trim(p_email)), trim(coalesce(p_full_name, '')), nullif(trim(p_company_name), ''), nullif(trim(p_phone), ''), trim(p_title), trim(p_description), p_category, p_budget_range, p_start_time, coalesce(p_is_company, true), coalesce(nullif(trim(p_source), ''), 'publicera'))
