...
SELECT pg_temp.check_ok(public.update_project_agreement(current_setting('qa.offer_one')::uuid, 'edit', 2, 'Revised synthetic scope', 'Synthetic acceptance test term')->'content'->>'buyer_confirmed_at' IS NULL, 'editing invalidates previous confirmation');
RESET ROLE;
SELECT set_config('request.jwt.claim.sub', pg_temp.qa_id(3)::text, true);
SET LOCAL ROLE authenticated;
