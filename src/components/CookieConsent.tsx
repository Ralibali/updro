...
const CookieConsent = () => {
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 p-2 sm:p-4" role="dialog" aria-modal="false" aria-labelledby="cookie-consent-title">
      <div className="max-w-3xl mx-auto bg-card border rounded-2xl shadow-lg p-3 sm:p-5 flex flex-col gap-3 sm:gap-4">
        <div className="text-sm text-foreground/80">
          <p id="cookie-consent-title" className="font-semibold text-foreground mb-1">Dina cookieval</p>
          <p className="hidden sm:block">Nödvändig lagring används för inloggning, säkerhet och ditt val. Statistik och marknadsföring är frivilliga och aktiveras endast för vald kategori. Läs vår <Link to="/integritetspolicy" className="text-primary hover:underline">integritetspolicy</Link> och <Link to="/cookies" className="text-primary hover:underline">cookiepolicy</Link>.</p>
          <p className="sm:hidden text-xs">Vi använder nödvändiga cookies. Statistik och marknadsföring är frivilliga. <Link to="/cookies" className="text-primary hover:underline">Läs mer</Link></p>
        </div>
        <div className={`${showDetails ? 'grid' : 'hidden'} sm:grid gap-3 sm:grid-cols-3`}>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          ...
          {showDetails ? (
            ...
            <Button variant="ghost" size="sm" className="rounded-xl w-full col-span-2 sm:hidden" onClick={() => setShowDetails(true)}>Anpassa val</Button>
          )}
        </div>
      </div>
    </div>
