...
import { Check, X, ShieldCheck, TrendingDown, Gift, MessageCircleReply, Timer, Ban } from 'lucide-react'
...
const USPSection = () => {
  return (
    <section className="py-20" id="hur-det-fungerar">
      <div className="container">
        <div className="text-center mb-14">
          <span className="inline-block rounded-full bg-accent/10 text-accent px-4 py-1.5 text-xs font-semibold mb-4">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          >
            <div className="space-y-3">
              {features.map((f) => (
                <div key={f.text} className="flex items-start gap-3 bg-card rounded-xl border p-4 transition-all hover:shadow-sm hover:border-primary/20">
            ...
            <div className="bg-card rounded-2xl border overflow-hidden shadow-sm">
              ...
              {comparison.map((row) => (
                <div key={row.label} className="grid grid-cols-3 border-t">
                  ...
                  <div className="p-3 md:p-4 text-center text-xs md:text-sm font-semibold text-primary flex items-center justify-center gap-1">
                  ...
                  <div className="p-3 md:p-4 text-center text-xs md:text-sm text-muted-foreground flex items-center justify-center gap-1">
                    {row.competitorBad ? <X className="h-4 w-4 text-destructive shrink-0" /> : null}
