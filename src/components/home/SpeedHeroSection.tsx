    ...
    <div className="container grid items-center gap-10 md:grid-cols-2">
      <div>
        ...
        <p className="mt-6 max-w-xl text-lg leading-relaxed text-muted-foreground">
          Beskriv ditt projekt så matchar Updro dig med relevanta svenska byråer. Max tre offerter, noga utvalda och enkla att jämföra.
        </p>
        ...
        <div className="mt-6 flex flex-wrap gap-4 text-sm text-muted-foreground">
          {['100% gratis', 'Granskade byråprofiler', 'Högst 3 offerter'].map(item => <span key={item} className="flex items-center gap-1.5"><Check className="h-4 w-4 text-accent" />{item}</span>)}
        </div>
      </div>
      <div className="rounded-3xl border bg-card p-7 shadow-xl">
        ...
        <div className="mt-6 flex gap-3 border-t pt-6"><ShieldCheck className="h-6 w-6 text-primary" /><div><h2 className="font-display text-xl font-bold">Tryggare byråval</h2><p className="mt-1 text-sm text-muted-foreground">Verifieringsstatus och företagsuppgifter visas tydligt i byråprofilen.</p></div></div>
      </div>
    </div>
  </section>
)
