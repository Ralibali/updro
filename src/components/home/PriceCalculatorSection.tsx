...
const PriceCalculatorSection = ({ defaultTypeId }: PriceCalculatorSectionProps) => {
  ...
  const guide = useMemo(() => PRICE_GUIDES.find(guideItem => guideItem.slug === projectType.guideSlug), [projectType])
      ...
      <div className="container">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 id="priskalkylator-rubrik" className="mt-3 font-display text-3xl md:text-5xl font-bold tracking-tight text-foreground [text-wrap:balance]">
            Få ett prisankare – <span className="text-accent">innan offerterna.</span>
          </h2>
          <p className="mt-4 text-muted-foreground leading-relaxed">
            Välj projekttyp och omfattning för ett första riktvärde från Updros prisguider. Det är ett planeringsunderlag, inte en offert eller prisgaranti.
          </p>
        </div>
        ...
        <div className="mt-10 max-w-3xl mx-auto border-2 border-foreground bg-card shadow-[6px_6px_0_0_hsl(var(--foreground))]">
          <div className="p-5 md:p-7 border-b-2 border-foreground">
            ...
            {levelCopy && <p className="mt-3 text-xs text-muted-foreground leading-relaxed"><span className="font-semibold text-foreground">{levelCopy.label} brukar innebära:</span> {levelCopy.includes}</p>}
          </div>

          <motion.div key={`${typeId}-${level}`} initial={reduce ? undefined : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="p-5 md:p-7 bg-background">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-start gap-3">
                <WalletCards className="mt-0.5 h-5 w-5 text-accent shrink-0" aria-hidden="true" />
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Riktvärde</p>
                  <p className="mt-1 font-display text-2xl md:text-3xl font-bold text-foreground tabular-nums">{cell?.range ?? '–'}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Clock3 className="mt-0.5 h-5 w-5 text-accent shrink-0" aria-hidden="true" />
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Typisk tidsplan i guiden</p>
            ...
            <p className="mt-4 text-xs text-muted-foreground">Offerter kan ligga både under och över riktvärdet beroende på scope, teknik, integrationsbehov och leveransupplägg.</p>
          </motion.div>
        </div>
      </div>
    </section>
