...
const baseRoutes = (): StaticSeoRoute[] => [
  {
    ...
    links: [
      ...
      { label: 'För byråer', href: '/for-byraer' },
  ...
  { path: '/publicera', title: 'Publicera uppdrag – få offerter från digitala byråer | Updro', description: 'Beskriv ditt digitala projekt gratis. Briefen granskas och högst tre relevanta byråer kan lämna offert.', h1: 'Beskriv ditt uppdrag och jämför offerter', priority: 0.9, changefreq: 'weekly' },
  { path: '/publicera/webbutveckling', title: 'Publicera webbutvecklingsuppdrag – jämför webbyråer | Updro', description: 'Beskriv webbutvecklingsuppdraget gratis. Briefen granskas och högst tre relevanta webbyråer kan lämna offert.', h1: 'Publicera ett webbuppdrag och jämför byråer', priority: 0.9, changefreq: 'weekly' },
  { path: '/byraer', title: 'Hitta digitala byråer i Sverige | Updro', description: 'Jämför webbyråer, SEO-byråer, e-handelsbyråer och digitala specialister i Sverige.', h1: 'Hitta rätt digital byrå', priority: 0.9, changefreq: 'weekly' },
  { path: '/priser', title: 'Priser för byråer – pay per lead eller månadskort | Updro', description: 'Se Updros transparenta priser för byråer och prisnivåer för digitala tjänster innan du jämför offerter.', h1: 'Priser för byråer och digitala tjänster', priority: 0.8, changefreq: 'weekly' },
  {
    path: '/for-byraer',
    title: 'Få fler kunder som webbyrå – digitala leads utan provision | Updro',
    description: 'Få digitala leads till din webbyrå eller digitalbyrå. Fem gratis lead-krediter, max tre byråer per uppdrag, brief före köp och 0 % projektprovision.',
    h1: 'Få fler relevanta uppdrag. Behåll hela projektvärdet.',
    priority: 0.9,
    changefreq: 'weekly',
    links: [
      { label: 'Se priser för byråer', href: '/priser' },
      { label: 'Skapa byråkonto', href: '/registrera/byra' },
      { label: 'Jämför Updro och Partna', href: '/partna-alternativ' },
  ...
  { path: '/hitta-webbyra', title: 'Hitta webbyrå – jämför upp till tre offerter | Updro', description: 'Hitta rätt webbyrå för hemsida, webbapp eller redesign. Beskriv projektet gratis och jämför upp till tre relevanta offerter.', h1: 'Hitta rätt webbyrå utan massutskick', priority: 0.9, changefreq: 'weekly' },
  { path: '/hitta-seo-byra', title: 'Hitta SEO-byrå – jämför upp till tre offerter | Updro', description: 'Jämför SEO-byråer inom teknisk SEO, innehåll och lokal synlighet. Beskriv behovet gratis och få upp till tre relevanta offerter.', h1: 'Hitta en SEO-byrå med rätt metod', priority: 0.9, changefreq: 'weekly' },
  { path: '/hitta-digital-byra', title: 'Hitta digital byrå – jämför upp till tre offerter | Updro', description: 'Hitta digital byrå för webb, annonsering, innehåll eller strategi. Beskriv projektet gratis och jämför upp till tre relevanta offerter.', h1: 'Hitta en digital byrå som passar uppdraget', priority: 0.9, changefreq: 'weekly' },
  {
  ...
  { path: '/redaktionell-policy', title: 'Redaktionell policy | Updro', description: 'Så arbetar Updro med granskning, kvalitet och transparens i guider och jämförelser.', h1: 'Redaktionell policy', priority: 0.4, changefreq: 'monthly' },
  { path: '/metod', title: 'Metod för jämförelser och guider | Updro', description: 'Läs hur Updro tar fram guider, jämförelser och rekommendationer för digitala projekt.', h1: 'Metod', priority: 0.4, changefreq: 'monthly' },
  ...
  { path: '/partna-alternativ', title: 'Partna pris 2026 & alternativ – Updro vs Partna', description: 'Jämför Updro och Partna för digitala uppdrag: leadpris, abonnemang, konkurrens per uppdrag, slagavgift och hur total kostnad kan skilja sig.', h1: 'Partna pris 2026 – och hur Updro skiljer sig', priority: 0.9, changefreq: 'weekly', links: [{ label: 'Alla jämförelser', href: '/jamfor' }, { label: 'För byråer', href: '/for-byraer' }, { label: 'Byt från Partna', href: '/for-byraer/byt-fran-partna' }] },
  { path: '/swivrr-alternativ', title: 'Alternativ till Swivrr – jämför Updro och Swivrr', description: 'Jämför Updro och Swivrr för digitala uppdrag: antal offerter, byråpriser, AI-brief, prisuppskattning, prisguider och byråprofiler.', h1: 'Updro eller Swivrr – vad passar bäst?', priority: 0.8, changefreq: 'monthly', links: [{ label: 'Alla jämförelser', href: '/jamfor' }] },
  { path: '/for-byraer/byt-fran-partna', title: 'Testa Updro som alternativ till Partna – för digitala byråer', description: 'Jämför pris, konkurrens per uppdrag och produktflöde. Börja med fem kostnadsfria lead-krediter och mät faktisk kostnad per vunnen affär.', h1: 'Testa Updro parallellt – byt först när siffrorna säger det', priority: 0.8, changefreq: 'monthly', links: [{ label: 'För byråer', href: '/for-byraer' }, { label: 'Saklig jämförelse Updro och Partna', href: '/partna-alternativ' }] },
  { path: '/support', title: 'Support | Updro', description: 'Kontakta Updros support för hjälp med uppdrag, offerter, konto eller fakturering.', h1: 'Support', priority: 0.4, changefreq: 'monthly' },
  ...
  return [
    ...
    ...SERVICE_CATEGORIES.map((service: any) => {
      ...
      return {
        ...
        intro: deep?.intro,
        sections: deep?.sections.map(section => ({ heading: section.heading, content: section.paragraphs.join("\n\n") })),
        cta: { label: "Jämför offerter gratis", href: seoLeadPath(service.slug) },
...
const urlset = (routes: StaticSeoRoute[]) => `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${routes.map(route => `  <url><loc>${abs(route.path)}</loc>${route.lastmod ? `<lastmod>${route.lastmod}</lastmod>` : ''}<changefreq>${route.changefreq}</changefreq><priority>${route.priority.toFixed(1)}</priority></url>`).join('\n')}\n</urlset>`
...
export const generateSitemapIndexXml = () => `<?xml version="1.0" encoding="UTF-8"?>\n<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${SITEMAP_SECTIONS.filter(sitemapSection => getIndexableSeoRoutes().some(route => section(route.path) === sitemapSection)).map(sitemapSection => `  <sitemap><loc>${SITE_URL}/sitemap-${sitemapSection}.xml</loc></sitemap>`).join('\n')}\n</sitemapindex>`
...
const staticMarkdown = (text: string) => text.split('\n\n').map(block => {
  ...
  if (lines.every(line => /^\d+\.\s/.test(line))) return `<ol>${lines.map(line => `<li>${staticText(line.replace(/^\d+\.\s/, ''))}</li>`).join('')}</ol>`
  if (lines.length > 1 && lines.every(line => line.trim().startsWith('|') && line.trim().endsWith('|'))) {
    const rows = lines.filter(line => !/^\|[\s|:-]+\|$/.test(line)).map(line => line.trim().slice(1, -1).split('|').map(cell => staticText(cell.trim())))
    return `<table><thead><tr>${rows[0].map(cell => `<th>${cell}</th>`).join('')}</tr></thead><tbody>${rows.slice(1).map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('')}</tbody></table>`
  }
  return `<p>${staticText(block)}</p>`
}).join('')
const staticSections = (route: StaticSeoRoute) => (route.sections || []).map(section => `<section><h2>${esc(section.heading)}</h2>${staticMarkdown(section.content)}</section>`).join('')
const staticSources = (route: StaticSeoRoute) => route.sources?.length ? `<section><h2>Källor och vidare läsning</h2><ul>${route.sources.map(source => `<li><a href="${esc(source.href)}">${esc(source.label)}</a></li>`).join('')}</ul></section>` : ''

const body = (route: StaticSeoRoute) => `${staticHeader()}<main id="static-seo-content" data-static-route="${esc(route.path)}">${staticBreadcrumbs(route)}<h1>${esc(route.h1)}</h1><p>${esc(route.intro || route.description)}</p>${convertCta(route)}${staticSections(route)}${staticSources(route)}${route.links?.length ? `<section><h2>Relaterade sidor</h2><ul>${route.links.map(link => `<li><a href="${esc(link.href)}">${esc(link.label)}</a></li>`).join('')}</ul></section>` : ''}${route.faq?.length ? `<section><h2>Vanliga frågor</h2>${route.faq.map(faq => `<article><h3>${esc(faq.q)}</h3><p>${esc(faq.a)}</p></article>`).join('')}</section>` : ''}</main>${staticFooter()}`
