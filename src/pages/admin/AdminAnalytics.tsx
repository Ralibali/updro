...
import { TrendingUp, Users, ClipboardList, CreditCard, Eye, ArrowUpRight, ArrowDownRight } from 'lucide-react'
...
const AdminAnalytics = () => {
  ...
  const fetchAll = async () => {
    ...
    setTotals({
      users: users || 0, suppliers: suppliers || 0, projects: projects || 0,
      offers: offers || 0, leads: leads || 0, reviews: reviews || 0,
    })
    ...
    setPrevTotals({
      users: prevUsers || 0, suppliers: prevSuppliers || 0,
      projects: prevProjects || 0, offers: prevOffers || 0,
    })

    // Registrations by day (last 30 days)
    const { data: allProfiles } = await supabase.from('profiles').select('created_at').gte('created_at', thirtyDaysAgo).order('created_at')
    if (allProfiles) {
      const byDay = groupByDay(allProfiles.map(p => p.created_at!))
      setRegistrations(byDay)
    }

    // Projects by day
    const { data: allProjects } = await supabase.from('projects').select('created_at').gte('created_at', thirtyDaysAgo).order('created_at')
    if (allProjects) {
      setProjectsByDay(groupByDay(allProjects.map(p => p.created_at!)))
    }

    // Category distribution
    const { data: catData } = await supabase.from('projects').select('category')
    if (catData) {
    ...
    // Budget distribution
    const { data: budgetData } = await supabase.from('projects').select('budget_range')
    if (budgetData) {
      const budgetMap = new Map<string, number>()
      budgetData.forEach(p => {
        const key = p.budget_range || 'Ej angiven'
        budgetMap.set(key, (budgetMap.get(key) || 0) + 1)
      })
      setBudgetDist(Array.from(budgetMap.entries()).map(([name, value]) => ({ name, value })))
    }

    // Plan distribution
    const { data: planData } = await supabase.from('supplier_profiles').select('plan')
    if (planData) {
      const planMap = new Map<string, number>()
      planData.forEach(p => {
        planMap.set(p.plan || 'none', (planMap.get(p.plan || 'none') || 0) + 1)
      })
      setPlanDist(Array.from(planMap.entries()).map(([name, value]) => ({ name, value })))
    }

    // Conversion stats
    const { data: projWithOffers } = await supabase.from('projects').select('id, offer_count, view_count')
    if (projWithOffers) {
      const withOffers = projWithOffers.filter(p => (p.offer_count || 0) > 0).length
      const avgOffers = projWithOffers.length > 0
        ? projWithOffers.reduce((s, p) => s + (p.offer_count || 0), 0) / projWithOffers.length
        : 0
      const avgViews = projWithOffers.length > 0
        ? projWithOffers.reduce((s, p) => s + (p.view_count || 0), 0) / projWithOffers.length
        : 0
      setConversionStats({
        totalProjects: projWithOffers.length,
        withOffers,
    ...
    // Top suppliers
    const { data: topSup } = await supabase.from('supplier_profiles')
      .select('id, slug, avg_rating, completed_projects, lead_credits, plan, is_verified')
    ...
    if (topSup) {
      ...
      setTopSuppliers(topSup.map(s => ({
        ...s,
        name: profileMap.get(s.id)?.company_name || profileMap.get(s.id)?.full_name || 'Okänd',
      })))
    }
  ...
  const StatCard = ({ label, value, icon: Icon, color, changePercent }: any) => (
