...
const AgencyCategoryPage = () => {
  ...
  const { agencies, loading, error, retry } = useAgencyDirectory({ category: category?.dbCategory })
  const [marketStats, setMarketStats] = useState<CategoryStats | null>(null)

  useEffect(() => {
    if (category) {
      setSEOMeta({
        title: `${category.name} – jämför byråer i Sverige | Updro`,
        description: `Hitta byråprofiler inom ${category.name.toLowerCase()}. Få gratis offerter via Updro.`,
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="container pb-16">
        <h2 className="font-display text-xl font-semibold mb-6">Byråprofiler inom {category.name.toLowerCase()} i Sverige</h2>
        {loading || error ? <DirectoryStatus loading={loading} error={error} retry={retry} /> : agencies.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground">Just nu visas inga verifierade byråprofiler i denna kategori. Beskriv ditt behov för att ge relevanta byråer möjlighet att svara.</p>
          </div>
        ) : (
      ...
      <SEOLeadCTA category={category.slug} categoryName={category.name.toLowerCase()} />
      <Footer />
    </div>
  )
}
